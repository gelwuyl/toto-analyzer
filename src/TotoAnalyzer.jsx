          ...
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-6">
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 border-b border-slate-800 pb-4">
                <div>
                  <h2 className="text-xl font-bold text-white">Empirical Dataset vs Hypergeometric Theory</h2>
                  <p className="text-xs text-slate-400">Comparing actual consecutive draw matches against mathematical calculations.</p>
                </div>
              </div>
        ...
        {activeTab === 'planner' && (
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            {plannerAnalysis && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
                  <h3 className="text-lg font-bold text-white mb-4 border-b border-slate-800 pb-3 flex items-center gap-2">🕒 Most Recent 10 Draws Overlap</h3>
                      ...
                      <div key={draw.drawNo} className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 rounded-lg border transition ${count >= 3 ? 'bg-amber-950/20 border-amber-900/50' : 'bg-slate-950/50 border-slate-800/50'}`}>
                        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                          <div className="flex flex-col">
                            <span className="text-sm font-semibold text-slate-200">Draw #{draw.drawNo}</span>
                            <span className="text-[10px] text-slate-500">{draw.date}</span>
                          </div>
                          <div className="flex flex-wrap items-center gap-1">
                            {draw.numbers.map(n => <span key={n} className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${matches.includes(n) ? 'bg-amber-400 text-slate-900 ring-1 ring-amber-300 shadow-sm scale-110 z-10' : 'bg-slate-800 text-slate-400'}`}>{n}</span>)}
                            {includeBonus && <div className="flex items-center ml-1 border-l border-slate-700 pl-1.5"><span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${matches.includes(draw.additional) ? 'bg-amber-400 text-slate-900 ring-1 ring-amber-300 shadow-sm scale-110 z-10' : 'bg-slate-800 text-slate-400 border border-slate-700'}`}>{draw.additional}</span></div>}
                          </div>
                        </div>
                        <div className="shrink-0 text-right">
                          <span className={`inline-block px-2.5 py-1 rounded text-xs font-bold ${count >= 4 ? 'bg-rose-500/20 text-rose-400' : count === 3 ? 'bg-amber-500/20 text-amber-400' : count > 0 ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-800 text-slate-500'}`}>{count} Match{count !== 1 && 'es'}</span>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-6 animate-in fade-in duration-300">
              <div>
                <h2 className="text-xl font-bold text-white">🎡 Wheeling System Generator</h2>
                <p className="text-sm text-slate-400 mt-1">
                  {betNumbers.length >= 6
                ...
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 shadow-inner sm:flex-1">
                      <h3 className="text-sm font-bold text-slate-200 mb-2">Pool ({betNumbers.length} numbers)</h3>
                      <p className="text-xs text-slate-500 mb-3 break-words">{betNumbers.join(', ')}</p>
                      <div className="flex gap-2">
                        <button onClick={() => setWheelMode('full')} className={`flex-1 py-1.5 text-xs font-semibold rounded transition ${wheelMode === 'full' ? 'bg-emerald-600 border border-emerald-500 text-white' : 'bg-slate-800 border border-slate-700 text-slate-400 hover:bg-slate-700'}`}>Full System</button>
                        <button onClick={() => setWheelMode('budget')} className={`flex-1 py-1.5 text-xs font-semibold rounded transition ${wheelMode === 'budget' ? 'bg-emerald-600 border border-emerald-500 text-white' : 'bg-slate-800 border border-slate-700 text-slate-400 hover:bg-slate-700'}`}>Abbreviated</button>
                      </div>
                    </div>
                  </div>
                  <button onClick={generateWheel} className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-sm font-bold transition shadow-lg shadow-emerald-600/30">
                    Generate Lines
                  </button>
                  {generatedWheel.length > 0 && (
                    <div className="border-t border-slate-800 pt-6 animate-in slide-in-from-bottom-2 duration-300">
                      <h3 className="text-md font-bold text-white mb-4">Generated Lines ({generatedWheel.length} combinations - ${generatedWheel.length})</h3>
                        ...
                        {generatedWheel.map((line, idx) => (
                          <div key={idx} className="bg-slate-950 border border-slate-800/80 p-2.5 rounded-lg flex justify-between items-center shadow-sm">
                            ...
                            <div className="flex gap-1.5">
                              {line.map(n => <span key={n} className="w-6 h-6 rounded-full bg-slate-800 flex items-center justify-center text-[10px] font-bold text-slate-300">{n}</span>)}
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              <div className="overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
                <div className="inline-block min-w-max">
                  ...
                  <div className="flex">
                    ...
                    {Array.from({length: 49}, (_, i) => i + 1).map(n => <div key={`hdr-${n}`} className="w-7 h-7 flex items-center justify-center text-[9px] font-bold text-slate-500">{n}</div>)}
                  </div>
                  {/* Matrix Rows */}
                  {Array.from({length: 49}, (_, i) => i + 1).map(rowNum => (
                    <div key={`row-${rowNum}`} className="flex">
                      <div className="w-8 h-7 flex items-center justify-end pr-2 text-[10px] text-slate-400 font-bold">{rowNum}</div>
                      {Array.from({length: 49}, (_, j) => j + 1).map(colNum => {
                        ...
                        return (
                          <div key={`cell-${rowNum}-${colNum}`} className="w-7 h-7 border border-slate-900/80 flex items-center justify-center text-[9px] font-mono transition-colors hover:border-amber-400" style={{
